#ifndef MODM_STM32H5_SPI_BASE_HPP
#define MODM_STM32H5_SPI_BASE_HPP
 ...
 *
 * STM32H5 uses the same extended SPI register map as STM32H7 (CFG1/CFG2/TXDR/RXDR),
 * but without the transfer-size-extension / reload feature present on H7.
 *
...
public:
	...
	Interrupt : uint32_t
	{
		...
		// No reload interrupt on STM32H5 (H7-only feature)
	};
	...
	StatusFlag : uint32_t
	{
		...
		// No reload status flag on STM32H5 (H7-only feature)
	...
	/// Data frame size programmed into CFG1.DSIZE (value = bits - 1).
	/// Full instances (SPI1–3) support up to Bit32; limited (SPI4–6) up to Bit16.
    // DSIZE bits in CFG1 register
    // STM32H5 supports several data sizes transfer (not only 8 - 16 - 32 as for other MCUS)
	enum class
	DataSize : uint32_t
	{
		Bit4 = 3,
    ...
    // DMAEN bits in CFG1 register for enabling DMA for TX and or RX
    ...
    // COMM bits in CFG2 register for selecting the communication mode
	enum class DuplexMode : uint32_t
	{
		FullDuplex = 0,                                  // TX and RX enabled
		TransmitOnly = SPI_CFG2_COMM_0,                  // TX enabled, RX disabled
		ReceiveOnly = SPI_CFG2_COMM_1,                   // TX disabled, RX enabled
		HalfDuplex = SPI_CFG2_COMM_1 | SPI_CFG2_COMM_0,  // TX and RX enabled, but only one at a time (same line)
		Mask = HalfDuplex
	};

    // SSM bits in CFG2 register for selecting the slave select mode
    // NSS either handled by hardware, or software 
    ...
    // SSIOP bits in CFG2 register for selecting the slave select polarity
    // Default: Active Low
    ...
    // CRCINI bits in CR1 register for selecting the CRC initialization
    // Default: All Zeros
...
#endif // MODM_STM32H5_SPI_BASE_HPP
